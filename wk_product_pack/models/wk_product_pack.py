...
class ProductPack(models.Model):
	...
	def create(self,vals):
		template_id = self.env['product.template'].browse(vals[0].get('wk_product_template'))
		for rec in vals:
			if template_id.product_variant_id.id == rec.get("product_id"):
				raise ValidationError('Can\'t add the same product {}'.format(template_id.name))
