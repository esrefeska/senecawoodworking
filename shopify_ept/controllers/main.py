...
class Main(http.Controller):
    ...
    @http.route(['/shopify_odoo_webhook_for_customer_create', '/shopify_odoo_webhook_for_customer_update'], csrf=False,
                method="POST", auth="public", type="json")
    def customer_create_or_update_webhook(self):
        ...
        if not res:
            return
        # Change code because last name or first name not get then it raise error.
        log_str = ""
        if res.get("first_name"):
            log_str = res.get("first_name")
        if res.get("last_name"):
            log_str = log_str + " " + res.get("last_name")
        _logger.info("%s call for Customer: %s", webhook_route, (log_str))
